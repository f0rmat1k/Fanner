...
Разъёмы, которые прокачиваются, но не сообщают обороты, скрыты: почти наверняка они
пустые. На этой плате их десять, а подключено шесть. Вентилятор на **0 % duty**
**не** скрывается — он остановлен, а не отсутствует, и прятать его значило бы убрать
с экрана вентилятор, который вы только что выключили. Разъём, доведённый нами до
полной остановки, помечается **STOPPED** и обводится рамкой: это единственный исход
перетаскивания ползунка, который может стоить железа.
...
- [x] Чтение вентиляторов, значений duty и температур; живые графики
- [x] Определение и объяснение отсутствующего драйвера или нехватки прав
- [x] Ползунок duty на каждый вентилятор и кнопка возврата управления BIOS
- [x] Температурный сторож
- [x] Кривые температура→обороты с сохранением между запусками
- [x] Профили, иконка в трее, запуск вместе с Windows
