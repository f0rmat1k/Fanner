...
Reads every fan header and temperature, drives any header manually, and runs
temperature curves. Profiles and a tray icon are not built yet — see
[Roadmap](#roadmap).
...
| Project | |
|---|---|
| `Fanner.Core` | Models, the `IHardwareBackend` contract, the polling service, rolling history, the curve engine, the watchdog, config persistence, and the simulated backend. No dependency on any hardware library. |
| `Fanner.Hardware.Windows` | LibreHardwareMonitor behind that contract, plus PawnIO detection. |
| `Fanner.App` | Avalonia UI, MVVM. |
| `Fanner.Core.Tests` | The parts worth pinning down: the empty-header heuristic, curve evaluation, smoothing, deadband and kickstart, watchdog thresholds, config round-tripping, monitor lifecycle and the release-on-exit guarantee. |
...
Bindings are saved to `%APPDATA%\Fanner\config.json` and restored on the next launch.
...
- [ ] Profiles, tray icon, run at startup
